...
class EmotivEpocX(EmotivBase):
    ...
    def get_hid_device(self):
        for device in hid.enumerate():
            print(f"Path: {device['path']}, Manufacturer: {device['manufacturer_string']}, Usage: {device['usage']}")
            if (device['manufacturer_string'] == 'Emotiv' and
                device['usage'] == 2 and
                device['path'] == self.device_path):
                return device

        raise Exception('Specified Emotiv Epoc X device not found')
    ...
    def get_stream_info(self):
        ...
        # include device path in stream name to make it unique
        stream_name = f"Epoc X {self.device_path.decode('utf-8')}"  
